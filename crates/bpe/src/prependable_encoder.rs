...
/// This implementation allows for efficient token counting when adding characters
...
impl<'a> PrependableEncoder<'a> {
    /// Creates a new PrependableEncoder with the given BPE encoding.
    ...
    /// Truncates the internal state to the specified length.
    ...
    /// Returns the current length of the internal state.
